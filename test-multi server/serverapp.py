...
def registerserver(recv):
  ...
  file.write(content)
  file.close()
  with open('serverkey.txt', 'rb') as in_file, open('serverkey1.txt"', 'wb') as out_file:
   encryptfile.encrypt(in_file, out_file)
  os.system('rm -f serverkey.txt')
def handlelogin(conn):
  while True:
    value1="serverhoang12022"
...
def authentication(input):
    with open('serverkey1.txt"', 'rb') as in_file, open('serverkey.txt', 'wb') as out_file:
      encryptfile.decrypt(in_file, out_file)
...
if(value.startswith("register")):
  ...
  sock = socket.socket()
  sock.bind(('192.168.111.181', 4456))
  sock.listen(5)
  while True:
