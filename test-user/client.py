...
data123 = sio.loadmat("kpca_vectest.mat")
a1234 = data123['W']
Sigma = 1
Nperm = 600
Nkernel = 2
...
def dangky(username_info,password_info):
    ...
    cv2.imwrite("test123.pgm", gray)
    os.system("wsl ./fjfxSample_static test123.pgm test123.ist")
    test="test123.ist"
    G_vecs=np.load("test123.npy")
    ft=FuzzyExtractor_1_parallel.istto(test)
    c=ft.dot(a1234)
    binary1=FuzzyExtractor_1_parallel.tranfor(c,G_vecs)
    os.chdir(r"C:\Users\MSI\OneDrive - Trường ĐH CNTT - University of Information Technology\Máy tính\test-user")
    os.system("del test123.ist")
    os.system("del test123.pgm")
    #data = sio.loadmat("tests/test_files/140_1.mat")
    #a1 = data['a']
    #a1=np.array(a1).flatten()
    a1=FuzzyExtractor_1_parallel.arraytosbin(binary1)
    fe = FuzzyExtractor_1_parallel.FuzzyExtractor()
    r, p = fe.gen(a1, locker_size=32, lockers=60000, confidence=None)
    FuzzyExtractor_1_parallel.savefile(p)
    os.system(r"copy file1.txt E:\file1.txt")
...
def login(username, passw):
    ...
    ft=FuzzyExtractor_1_parallel.istto(test)
    c=ft.dot(a1234)
    binary2=FuzzyExtractor_1_parallel.tranfor(c,G_vecs)
    os.chdir(r"C:\Users\MSI\OneDrive - Trường ĐH CNTT - University of Information Technology\Máy tính\test-user")
    os.system("del test123.ist")
    os.system("del test123.pgm")
    b1=FuzzyExtractor_1_parallel.arraytosbin(binary2)
    fe = FuzzyExtractor_1_parallel.FuzzyExtractor()
    os.system(r"copy E:\file1.txt file1.txt")
...
def login_user():
    username_info = username.get()
    password_info = password.get()
    f = open(r"E:\filesave1.txt", "r")
    line=f.readlines()
    nameser=line[0]
    if(nameser=="serverhoang22022\n"):
        ip_info1="192.168.111.180"
    if(nameser=="serverhoang12022\n"):
        ip_info1="192.168.111.181"
    PORT=4456
    HOST=ip_info1
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
...
def newpassword():
    ...
    ft=FuzzyExtractor_1_parallel.istto(test)
    c=ft.dot(a1234)
    binary3=FuzzyExtractor_1_parallel.tranfor(c,G_vecs)
    os.chdir(r"C:\Users\MSI\OneDrive - Trường ĐH CNTT - University of Information Technology\Máy tính\test-user")
    os.system("del test123.ist")
    os.system("del test123.pgm")
    passw=password.get()
    a1=FuzzyExtractor_1_parallel.arraytosbin(binary3)
    fe = FuzzyExtractor_1_parallel.FuzzyExtractor()
    ...
    file.write(content)
    file.close()
    Label(update1_screen, text="Success", fg="green", font=("calibri", 11)).pack()
def updatapassword1():
    global update1_screen
    update1_screen = Toplevel(main_screen)
    update1_screen.title("Register")
    update1_screen.geometry("300x250")
    global password
    global password_entry
    password = StringVar()
    Label(update1_screen, text="Please enter details below", bg="blue").pack()
    Label(update1_screen, text="").pack()
    password_lable = Label(update1_screen, text="Password * ")
    password_lable.pack()
    password_entry = Entry(update1_screen, textvariable=password, show='*')
    password_entry.pack()
    Label(update1_screen, text="").pack()
    Button(update1_screen, text="Register", width=10, height=1, bg="blue", command = newpassword).pack() 
def updatepassword_user():
    username_info = username.get()
    password_info = password.get()
    global user
    user=username_info
    c_i1,d_i=login(username_info,password_info)
    global d_i1
    d_i1=d_i
    f = open(r"E:\filesave.txt", "r")
    line=f.readlines()
    ...
    f.close()
    if(c_i1!=c_i):
        Label(updatepassword_screen, text="Failed", fg="green", font=("calibri", 11)).pack()
    else:
        updatapassword1()
def register():
...
def login_1():
    ...
    password_entry.pack()
    #ip1_lable = Label(login_screen, text="Ip * ")
    #ip1_lable.pack()
    #ip1_entry = Entry(login_screen, textvariable=ip1)
    #ip1_entry.pack()
    Label(login_screen, text="").pack()
    Button(login_screen, text="Login", width=10, height=1, bg="blue", command = login_user).pack()
def updatepassword():
    global updatepassword_screen
    updatepassword_screen = Toplevel(main_screen)
    updatepassword_screen.title("update-password")
    updatepassword_screen.geometry("300x250")
    global username
    global password
    global username_entry
    global password_entry
    username = StringVar()
    password = StringVar()
    Label(updatepassword_screen, text="Please enter details below", bg="blue").pack()
    Label(updatepassword_screen, text="").pack()
    username_lable = Label(updatepassword_screen, text="Username * ")
    username_lable.pack()
    username_entry = Entry(updatepassword_screen, textvariable=username)
    username_entry.pack()
    password_lable = Label(updatepassword_screen, text="Password * ")
    password_lable.pack()
    password_entry = Entry(updatepassword_screen, textvariable=password, show='*')
    password_entry.pack()
    Label(updatepassword_screen, text="").pack()
    Button(updatepassword_screen, text="Login", width=10, height=1, bg="blue", command = updatepassword_user).pack()
def main_account_screen():
    ...
    Label(text="").pack()
    Button(text="Update-password", height="2", width="30", command=updatepassword).pack()
